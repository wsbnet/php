 ...
 * @return 分页
 */
function mypages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	if(defined('URLRULE') && $urlrule == '') {
		$urlrule = URLRULE;
		$array = $GLOBALS['URL_ARRAY'];
	} elseif($urlrule == '') {
	...
	if($num > $perpage) {
		...
		$multipage .= '<ul>';
		if($curr_page>0) {
			$multipage .= ' <li><a href="'.pageurl($urlrule, $curr_page-1, $array).'" >Prev</a></li>';
			if($curr_page==1) {
		...
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
				$multipage .= ' <li>..<a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></li> <li><a href="'.pageurl($urlrule, $curr_page+1, $array).'" >Next</a></li>';
			} else {
				$multipage .= ' <li><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></li> <li><a href="'.pageurl($urlrule, $curr_page+1, $array).'" >Next</a></li>';
			}
		} elseif($curr_page==$pages) {
			$multipage .= ' <li class="active"><a>'.$pages.'</a></li> <li><a href="'.pageurl($urlrule, $curr_page, $array).'" >Next</a></li>';
		} else {
			$multipage .= ' <li><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></li> <li><a href="'.pageurl($urlrule, $curr_page+1, $array).'" >Next</a></li>';
		}
	}
 ...
 * @param $symbol 栏目间隔符
 */
function catpos($catid, $symbol=' <span class="divider">/</span> ',$title=''){
	$category_arr = array();
	...
	foreach($arrparentid as $catid) {
		...
		if($cats==$i){
			if(!empty($title)){
				$pos .= '<li><a href="'.$url.'">'.ucfirst(strtolower($category_arr[$catid]['catname'])).'</a>'.$symbol.'</li><li class="active">'.$title.'</li>';
			}else{
				$pos .= '<li class="active">'.ucfirst(strtolower($category_arr[$catid]['catname'])).'</li>';
			}
		}else{
			if($category_arr[$catid]['child']!=0){
				$pos .= '<li>'.ucfirst(strtolower($category_arr[$catid]['catname'])).$symbol.'</li>';
			}else{
				$pos .= '<li><a href="'.$url.'">'.ucfirst(strtolower($category_arr[$catid]['catname'])).'</a>'.$symbol.'</li>';
			}
		}
	}
	return $pos;
